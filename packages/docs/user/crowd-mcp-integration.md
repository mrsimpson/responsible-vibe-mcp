...
### 1. Set Up Crowd MCP Server

Install and configure [crowd-mcp](https://github.com/mrsimpson/crowd-mcp) for multi-agent orchestration.

### 2. Configure Agent Roles

The `.crowd/agents/` directory contains three pre-configured agent roles:

- **business-analyst**: Requirements gathering and specification
...
```yaml
env:
  VIBE_ROLE: business-analyst # Agent's role in the team
  VIBE_WORKFLOW_DOMAINS: sdd-crowd # Access to collaborative workflows
```
